...
| 10 | 収束(close) | 全成果物 | 検証記録(PR 本文)+ knowledge dump + merge | 検証記録が PR に書かれ、work を片付けて merge・ブランチ削除済み | [git.md](git.md) |
...
してよい。その場合、検証記録の代わりに「次の bolt への引き継ぎメモ」を issue(または
PR 本文)に書く。

## リリース(bolt 横断の節目)

フェーズ 10(収束)は bolt を閉じるだけで、外に出す判断は含まない。複数の bolt を束ねて
利用者に届ける行為は**リリース**として [release.md](release.md) に従う(判定チェックリスト・
タグ・リリースノート・ロールバック。GO 判断は `escalation_to`)。
...
- 各 bolt は一時作業場 `docs/work/<topic>/` で成果物を育てる(日付は使わない):

```
docs/work/<topic>/
├── intent.md         # 目的・課題・対象ユーザー(フェーズ1。骨子は issue にも)
...
収束(フェーズ10)では、検証記録を PR 本文に書き、知見を docs/knowledge/ に dump し、
恒久成果物を docs/design/ 等へ移した上で、この一時作業場は**削除して merge** する
([git.md](git.md))。ファイルテンプレートは [docs/work/README.md](../work/README.md)。
