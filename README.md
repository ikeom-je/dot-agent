...
| `tools/` | check-links.sh: リンク検証(このリポジトリの正式グリーン判定。既知の制限: コードブロック内のリンク記法も検査対象のため、実在しない例をリンク記法で書かない)。new-bolt.sh: bolt 開始(worktree・ブランチ・work dir)の一括作成。search-knowledge.sh: 知見の3層(insights.md・git log・issue/PR)横断検索 |
